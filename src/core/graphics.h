...
//   `requested_size` is the minimum size the user needs the buffer to be, but the actual size of
//     the buffer on the device may be different due to alignment requirements. To retrieve the
//     actual size of the buffer use graphics_buffer_device_size()
...
//   Note, that much like the create function, 'new_size' is the minimum size the user needs the buffer
//     to be but due to alignment requirements the actual device size of the buffer may be larger. 
...
//   If the given buffer is set to persistent mapping a warning is given and the function returns
//     the mapped data pointer. If the buffer was already mapped then a warning is given and the
//     function returns the mapped data pointer.
//   If the mapping is successful then a pointer to the mapped data is returned. Otherwise 0 is returned.
...
//   If the given buffer is set to persistent mapping then a warning is given and the function returns.
...
namespace graphics{
	struct Buffer : public GraphicsBuffer{
		static Buffer* create(void* data, u64 requested_size, GraphicsBufferUsage usage, GraphicsMemoryPropertyFlags properties, GraphicsMemoryMappingBehavoir mapping_behavior){ return (Buffer*)graphics_buffer_create(data, requested_size, usage, properties, mapping_behavior); }
...
// Uploads the given pixels to the given GraphicsImage within the region specified by 'offset' and 'extent'.
//   This function does nothing and emits a warning if either axis in 'extent' is 0, the region
//     is completely outside the bounds of the image.
...
//   The bindings each describes the type of data being referenced as well as what shader stages it
//     is to be used in and which binding that resource should be attached to.
...
namespace graphics{
	...
	struct DescriptorSetLayout : public GraphicsDescriptorSetLayout{
		static DescriptorSetLayout* allocate(){ return (DescriptorSetLayout*)graphics_descriptor_set_layout_allocate(); }
	...
	struct DescriptorSet : public GraphicsDescriptorSet{
		static DescriptorSet* allocate(){ return (DescriptorSet*)graphics_descriptor_set_allocate(); }
...
namespace graphics{
	struct PipelineLayout : public GraphicsPipelineLayout{
		static PipelineLayout* allocate(){ return (PipelineLayout*)graphics_pipeline_layout_allocate(); }
	...
	struct Pipeline : public GraphicsPipeline{
		static Pipeline* allocate(){ return (Pipeline*)graphics_pipeline_allocate(); }
...
namespace graphics{
	struct RenderPass : public GraphicsRenderPass{
		...
		static RenderPass* of_window_presentation_frames(Window* window){ return (RenderPass*)graphics_render_pass_of_window_presentation_frames(window); }
	};
} //namespace graphics
#endif //#if COMPILER_FEATURE_CPP
...
typedef struct GraphicsCommand{
	...
	union{
		...
		struct{ // set_viewport, set_scissor
			vec2 offset;
			vec2 extent;
