...
namespace Storage{
	...
	//NOTE calls `Render::LoadMesh()` after creation
	...
	inline char* MeshName(u32 index){ return DeshStorage->meshes[index]->name; };
	...
	//NOTE `type`, `filter`, and `uvMode` arguments determine usage in `Render`
	//NOTE `generateMipmaps` determines whether `Render` should generate and use mipmaps for rendering this texture
	//NOTE the `pixels` var of the created `Texture` object is freed from memory unless `keepLoaded` is true
	//NOTE calls `Render::LoadTexture()` after creation
	...
	//NOTE `generateMipmaps` determines whether `Render` should generate and use mipmaps for rendering this texture
	//NOTE calls `Render::LoadTexture()` after creation
	pair<u32,Texture*> CreateTextureFromMemory(void* data, const char* name, s32 width, s32 height, ImageFormat format, TextureType type = TextureType_2D, TextureFilter filter = TextureFilter_Nearest, TextureAddressMode uvMode = TextureAddressMode_Repeat, b32 generateMipmaps = true);
	...
	//TODO change the return type to cstring
	inline char*       TextureName(u32 index){ return DeshStorage->textures[index]->name; };
	...
	//NOTE calls `Render::LoadMaterial()` after creation
	...
	//NOTE calls `Render::LoadMaterial()` after creation
	pair<u32,Material*> CreateMaterialFromFile(const char* filename);
	...
	//TODO change the return type to cstring
	inline char* MaterialName(u32 index){ return DeshStorage->materials[index]->name; };
	...
	//NOTE creates a new `Mesh` object in `Storage` from an `OBJ` file if no `MESH` file is found or `forceLoadOBJ` is true
	//NOTE calls `Render::LoadMesh()` if new mesh data is generated
	...
	//returns the name of the `Model` object at `index` in `Storage`
	//TODO change the return type to cstring
	inline char* ModelName(u32 index){ return DeshStorage->models[index]->name; };
