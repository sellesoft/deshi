...
local Logger* console_logger;
local b32 console_open_pressed = false;
local UIWindow* console_window;
local UIWindowFlags console_window_flags = UIWindowFlags_NoMove | UIWindowFlags_NoResize | UIWindowFlags_NoScrollX;
local b32 console_scroll_to_bottom = false;
local Stopwatch console_open_timer;
local Arena* console_chunk_render_arena;
...
void console_send_to_logger(str8 message, b32 newline){DPZoneScoped;
	b32 restore_mirror  = console_logger->mirror_to_console;
	b32 restore_tag     = console_logger->ignore_tags;
	b32 restore_newline = console_logger->auto_newline;
	b32 restore_track   = console_logger->track_caller;
	console_logger->mirror_to_console = false;
	console_logger->ignore_tags       = true;
	console_logger->auto_newline      = newline;
	console_logger->track_caller      = false;
	logger_format_log(str8_lit(__FILE__),__LINE__,str8{},LogType_Normal,str8_lit("%.*s"),message.count,message.str);
	console_logger->mirror_to_console = restore_mirror;
	console_logger->ignore_tags       = restore_tag;
	console_logger->auto_newline      = restore_newline;
	console_logger->track_caller      = restore_track;
}
...
void console_parse_message(str8 message){DPZoneScoped;
	...
	u64 log_offset = console_logger->file->bytes;
...
void console_init(){
	...
	console_logger->mirror_to_console = true;
...
void console_update(){
	...
	if(change_input){
		...
		if(console.input_history_index != -1){
			...
			u64 restore = console_logger->file->cursor;
			for(;;){
			...
			file_cursor(console_logger->file, restore);
		}
	}
	...
	if(console.console_dim.y > 0){
		...
		UI::BeginChild(str8_lit("deshiConsoleTerminal"), (console_window->dimensions - 2*style.windowMargins).yAdd(-(1.3*style.fontHeight + style.itemSpacing.y)), console_window_flags);{
			...
			forI(console.dictionary.count){
				...
				file_cursor(console_logger->file, console.dictionary[i].start);
				file_read(console_logger->file, console_chunk_render_arena->start, console.dictionary[i].size);
	...
	switch(new_state){
		case ConsoleState_Closed:{
			if(console.state == ConsoleState_Popout){
				console.console_pos = console_window->position;
				console.console_dim = console_window->dimensions;
			}
