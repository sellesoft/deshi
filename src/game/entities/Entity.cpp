		...
		switch(header){
			...
			case(Header::DOOR):{
				if(kv.first == "open"){ 
					if(kv.second == "true" || kv.second == "1"){
						door->isOpen = true;
					}else if(kv.second == "false" || kv.second == "0"){
						door->isOpen = false;
					}else{
						ERROR("Error parsing '",filepath,"' on line '",line_number,"'! Invalid boolean value: ", kv.second);
					}
				}
			...
			case(Header::MESH):{
				if(kv.first == "name"){ 
					...
					if(kv.second == "true" || kv.second == "1"){
						mesh->mesh_visible = true;
					}else if(kv.second == "false" || kv.second == "0"){
						mesh->mesh_visible = false;
					}else{
						ERROR("Error parsing '",filepath,"' on line '",line_number,"'! Invalid boolean value: ", kv.second);
					}
				}
			...
			case(Header::PHYSICS):{
				if     (kv.first == "velocity")        { phys->velocity = get_vec3(kv.second); }
					...
					if(kv.second == "true" || kv.second == "1"){
						phys->staticRotation = true;
					}else if(kv.second == "false" || kv.second == "0"){
						phys->staticRotation = false;
					}else{
						ERROR("Error parsing '",filepath,"' on line '",line_number,"'! Invalid boolean value: ", kv.second);
					}
				}
