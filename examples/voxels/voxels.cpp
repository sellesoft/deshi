...
#define ASTEROIDS_COUNT 8192
	...
	deshi_loop_start();{
		...
		uiImmediateB();{
			...
			uiTextM(to_dstr8v(deshi_temp_allocator, (int)F_AVG(100,1000/DeshTime->deltaTime),        " fps"));
			uiTextM(to_dstr8v(deshi_temp_allocator, (int)F_AVG(100,render_get_stats()->renderTimeMS)," ms render"));
			uiTextM(to_dstr8v(deshi_temp_allocator, render_get_stats()->totalVoxels,                 " voxels"));
			uiTextM(to_dstr8v(deshi_temp_allocator, render_get_stats()->totalTriangles,              " triangles"));
			uiTextM(to_dstr8v(deshi_temp_allocator, render_get_stats()->totalVertices,               " vertices"));
			uiTextM(to_dstr8v(deshi_temp_allocator, render_get_stats()->totalIndices,                " indices"));
			uiTextM(to_dstr8v(deshi_temp_allocator, camera.position));
			uiTextM(to_dstr8v(deshi_temp_allocator, camera.rotation));
			uiItemE();
		}uiImmediateE();
	}deshi_loop_end();
	deshi_cleanup();
